...
const LOVABLE_REPO = "JoaoRG-lab/rhema-care-flow-8d712b43";
const ALLOWED_REPOS = new Set([CANONICAL_REPO, LOVABLE_REPO]);
...
function assertAllowedRepo(repo: string) {
  if (!ALLOWED_REPOS.has(repo)) {
    throw new Error(`Refusing to edit unauthorized repo: ${repo}`);
  }
}
...
serve(async (req) => {
  ...
  try {
    ...
    if (!validation.changed) return jsonResponse({ success: true, dryRun, changed: false, validation, repo, baseBranch, targetFile: body.targetFile });
