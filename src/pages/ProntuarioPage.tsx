  ...
  return (
    <div className="space-y-3 rounded-xl border border-purple-200 bg-purple-50/60 p-3 dark:border-purple-900 dark:bg-purple-950/20">
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        ...
        <button type="button" onClick={() => onUseText(formatted.text)} className="rounded-lg bg-purple-600 px-3 py-2 text-xs font-semibold text-white hover:bg-purple-700">
          Usar texto gerado
        </button>
      </div>
      ...
      <div className="rounded-lg border border-purple-100 bg-white p-3 dark:border-purple-900 dark:bg-gray-900">
        ...
        <select defaultValue="" onChange={(event) => { if (event.target.value) applyTemplate(event.target.value); event.currentTarget.value = ''; }} className="w-full rounded-lg border border-gray-200 bg-gray-50 px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-purple-500 dark:border-gray-700 dark:bg-gray-800">
      ...
      {items.map((item, index) => {
        ...
        return (
          <div key={index} className="rounded-lg border border-purple-100 bg-white p-3 shadow-sm dark:border-purple-900 dark:bg-gray-900">
            ...
            {itemAlerts.length > 0 && <div className="mt-2 space-y-1">{itemAlerts.map((alert) => <div key={`${alert.title}-${alert.message}`} className={`rounded-lg border p-2 text-xs ${alertClass(alert.level)}`}><strong>{alert.title}:</strong> {alert.message}</div>)}</div>}
          </div>
        );
      })}

      <button type="button" onClick={addItem} className="rounded-lg border border-purple-200 px-3 py-2 text-xs font-semibold text-purple-700 hover:bg-purple-100 dark:border-purple-900 dark:text-purple-300">Adicionar medicamento</button>
      ...
      <div className={`rounded-lg border p-2 text-xs ${formatted.canFinalize ? 'border-emerald-200 bg-emerald-50 text-emerald-800' : 'border-amber-200 bg-amber-50 text-amber-800'}`}>
        {formatted.summary}{!formatted.canFinalize && <span className="ml-1 font-semibold">Revise alertas críticos antes de finalizar.</span>}
      </div>

      {alerts.length > 0 && <div className="space-y-1">{alerts.map((alert) => <div key={`${alert.level}-${alert.title}-${alert.message}`} className={`rounded-lg border p-2 text-xs ${alertClass(alert.level)}`}><strong>{alert.title}:</strong> {alert.message}</div>)}</div>}

      {items.some(hasPrescriptionContent) && (
        <PrescriptionPrintPreview patientName="Paciente do prontuário" items={items} />
      )}
    </div>
  );
}
...
export function ProntuarioPage({ patientId, patientName }: ProntuarioPageProps) {
  ...
  async function handleAdd(e: React.FormEvent) {
    ...
    const nextContent = entryType === 'prescricao' && !content.trim() && prescriptionItems.some(hasPrescriptionContent) ? generated?.text ?? '' : content.trim();
    if (!nextContent.trim() || !user) return;
    setSaving(true);
    setSaveError(null);
    const { error: addErr } = await addEntry({ patient_id: patientId, author_id: user.id, entry_type: entryType, content: nextContent, visit_id: null });
      ...
      <div className="space-y-5 max-w-3xl">
        ...
        {showForm && (
          <form onSubmit={handleAdd} className="bg-white dark:bg-gray-900 rounded-xl border border-teal-200 dark:border-teal-800 p-4 space-y-3 shadow-sm">
            ...
            <div className="flex flex-wrap gap-1.5">{ENTRY_TYPES.map((t) => <button key={t.value} type="button" onClick={() => { setEntryType(t.value); setSaveError(null); }} className={`px-3 py-1 rounded-full text-xs font-medium transition-colors ${entryType === t.value ? 'bg-teal-600 text-white' : 'bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-400 hover:bg-gray-200 dark:hover:bg-gray-700'}`}>{t.label}</button>)}</div>
            {entryType === 'prescricao' && <PrescriptionComposer items={prescriptionItems} onChange={setPrescriptionItems} onUseText={setContent} />}
            <div><label htmlFor="entry-content" className="sr-only">Conteúdo da entrada</label><textarea id="entry-content" value={content} onChange={(e) => setContent(e.target.value)} rows={entryType === 'prescricao' ? 8 : 5} placeholder={`Registre a ${currentTypeLabel.toLowerCase()}...`} className="w-full rounded-lg border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800 px-3 py-2 text-sm text-gray-900 dark:text-gray-100 placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-teal-500 resize-y" /></div>
            {saveError && <p className="text-xs text-red-500">{saveError}</p>}
            <div className="flex gap-2 justify-end"><button type="button" onClick={() => { setShowForm(false); setContent(''); setPrescriptionItems([{ ...emptyPrescriptionItem }]); setSaveError(null); }} className="px-4 py-2 rounded-lg border border-gray-200 dark:border-gray-700 text-sm text-gray-600 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors">Cancelar</button><button type="submit" disabled={(!content.trim() && !(entryType === 'prescricao' && prescriptionItems.some(hasPrescriptionContent))) || saving} className="px-4 py-2 rounded-lg bg-teal-600 hover:bg-teal-700 disabled:opacity-50 text-white text-sm font-medium transition-colors">{saving ? 'Salvando...' : 'Salvar'}</button></div>
          </form>
        )}

        {loading && <div className="space-y-3">{[...Array(4)].map((_, i) => <div key={i} className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-700 p-4 animate-pulse"><div className="h-3 w-20 bg-gray-100 dark:bg-gray-800 rounded mb-3" /><div className="space-y-2"><div className="h-3 w-full bg-gray-100 dark:bg-gray-800 rounded" /><div className="h-3 w-4/5 bg-gray-100 dark:bg-gray-800 rounded" /></div></div>)}</div>}
        {!loading && error && <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-xl p-4"><p className="text-sm text-red-600 dark:text-red-400">{error}</p><button onClick={refetch} className="mt-1 text-xs text-red-500 hover:underline">Tentar novamente</button></div>}
        {!loading && !error && entries.length === 0 && <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-700 p-12 text-center"><svg width="36" height="36" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" className="mx-auto text-gray-300 dark:text-gray-600 mb-3" aria-hidden="true"><path d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg><p className="text-sm text-gray-500 dark:text-gray-400">Nenhuma entrada no prontuário ainda.</p><button onClick={() => setShowForm(true)} className="mt-2 text-xs text-teal-600 hover:underline">Adicionar primeira entrada</button></div>}
        {!loading && entries.length > 0 && <div className="space-y-3">{entries.map((entry) => <article key={entry.id} className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-700 p-4 shadow-sm"><div className="flex items-start justify-between gap-2 mb-2"><div className="flex items-center gap-2 flex-wrap"><span className={`px-2 py-0.5 rounded-full text-xs font-medium ${TYPE_COLORS[entry.entry_type] ?? TYPE_COLORS.outro}`}>{ENTRY_TYPES.find(t => t.value === entry.entry_type)?.label ?? entry.entry_type}</span>{entry.author && <span className="text-xs text-gray-500 dark:text-gray-400">{entry.author.full_name}</span>}</div><time dateTime={entry.created_at} className="text-xs text-gray-400 dark:text-gray-500 flex-shrink-0">{new Date(entry.created_at).toLocaleString('pt-BR', { day: '2-digit', month: '2-digit', year: 'numeric', hour: '2-digit', minute: '2-digit' })}</time></div><p className="text-sm text-gray-800 dark:text-gray-200 leading-relaxed whitespace-pre-wrap">{entry.content}</p></article>)}</div>}
      </div>
    </AppShell>
  );
}
