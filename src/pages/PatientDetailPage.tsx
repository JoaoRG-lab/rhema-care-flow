      ...
      <div className="max-w-3xl space-y-5">
          ...
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 flex-wrap mb-1">
                ...
                {!patient.active && (
                  <span className="px-2 py-0.5 rounded-full text-xs bg-gray-100 dark:bg-gray-800 text-gray-500">Inativo</span>
                )}
              </div>
            ...
            <div className="flex gap-2 flex-shrink-0">
              <a
                ...
                className="px-3 py-1.5 rounded-xl border border-gray-200 dark:border-gray-700 text-xs font-medium text-gray-600 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors"
              >
                Prontuário
              </a>
              <a
          ...
          <div className="mt-4 pt-4 border-t border-gray-100 dark:border-gray-800 grid sm:grid-cols-3 gap-3">
            {patient.phone_number && (
              <div>
                ...
                <a href={`tel:${patient.phone_number}`} className="text-sm text-gray-900 dark:text-gray-100 hover:text-teal-600 transition-colors">
                  {patient.phone_number}
                </a>
              </div>
            )}
            {patient.email && (
              <div>
                ...
                <a href={`mailto:${patient.email}`} className="text-sm text-gray-900 dark:text-gray-100 hover:text-teal-600 transition-colors truncate block">
                  {patient.email}
                </a>
              </div>
            )}
            {patient.date_of_birth && (
              <div>
                <p className="text-xs text-gray-400 dark:text-gray-500 mb-0.5">Nascimento</p>
                <p className="text-sm text-gray-900 dark:text-gray-100">
                  {new Date(patient.date_of_birth + 'T00:00:00').toLocaleDateString('pt-BR')}
                </p>
              </div>
            )}
          </div>
          ...
          {TABS.map((t) => (
            ...
            >
              ...
              {t.count !== undefined && t.count > 0 && (
                <span className="ml-1.5 text-xs bg-teal-100 dark:bg-teal-900/40 text-teal-700 dark:text-teal-300 px-1.5 py-0.5 rounded-full">
                  {t.count}
                </span>
              )}
            </button>
        ...
        {tab === 'overview' && (
          <div className="grid sm:grid-cols-3 gap-4">
            <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-800 p-4">
              <p className="text-xs text-gray-400 mb-1">Entradas no prontuário</p>
              <p className="text-2xl font-bold text-gray-900 dark:text-gray-100">
                {pronLoading ? '—' : entries.length}
              </p>
            </div>
            <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-800 p-4">
              <p className="text-xs text-gray-400 mb-1">Visitas totais</p>
              <p className="text-2xl font-bold text-gray-900 dark:text-gray-100">
            ...
            <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-800 p-4">
              ...
              <p className="text-sm font-semibold text-gray-900 dark:text-gray-100">
                {visitsLoading ? '—' : visits[0]
                  ? new Date(visits[0].scheduled_at).toLocaleDateString('pt-BR')
                  : 'Sem visitas'}
              </p>
            </div>
          </div>
        ...
        {tab === 'prontuario' && (
          <div className="text-center py-8 text-sm text-gray-500">
            <a href={`/patients/${patient.id}/prontuario`} className="text-teal-600 hover:underline">
          ...
          <div className="space-y-3">
            {visitsLoading && (
              <div className="space-y-2">
                {[...Array(3)].map((_, i) => (
                  <div key={i} className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-700 p-4 animate-pulse h-16" />
                ))}
              </div>
            )}
              ...
              <div key={v.id} className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 p-4 flex items-center gap-3">
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-gray-900 dark:text-gray-100">
                    {new Date(v.scheduled_at).toLocaleString('pt-BR', { day: '2-digit', month: '2-digit', year: 'numeric', hour: '2-digit', minute: '2-digit' })}
                  </p>
                  {v.chief_complaint && (
                    <p className="text-xs text-gray-500 truncate">{v.chief_complaint}</p>
                  )}
                </div>
                <span className={`px-2.5 py-0.5 rounded-full text-xs font-medium flex-shrink-0 ${STATUS_COLORS[v.status]}`}>
