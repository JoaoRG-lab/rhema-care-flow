  ...
  return (
    <div className="flex items-center gap-3">
      ...
      <input
        ...
        value={value}
        onChange={(event) => onChange(clamp(Number(event.target.value), min, max))}
        className="w-24 rounded-xl border border-gray-200 bg-gray-50 px-3 py-2 text-right text-sm text-gray-900 outline-none focus:ring-2 focus:ring-teal-500"
      ...
      <div className="max-w-6xl space-y-6">
        <div className="flex flex-col gap-2 md:flex-row md:items-end md:justify-between">
          <div>
            ...
            <p className="max-w-3xl text-sm text-gray-500">
              Calculadoras reumatológicas para apoio longitudinal. Use como ferramenta clínica auxiliar: classificação não substitui diagnóstico, exame físico ou julgamento presencial.
            </p>
          ...
          {latest && (
            <div className="rounded-2xl border border-teal-100 bg-teal-50 px-4 py-3 text-sm text-teal-800">
              Último {activeScore.name}: <strong>{latest.score_value}</strong> · {new Date(latest.created_at).toLocaleDateString('pt-BR')}
            </div>
          )}
        </div>
        ...
        <div className="grid gap-4 lg:grid-cols-[280px_minmax(0,1fr)]">
          <aside className="space-y-4">
            {areas.map((area) => (
              <div key={area} className="rounded-2xl border border-gray-200 bg-white p-3 shadow-sm">
                <h2 className="mb-2 text-xs font-bold uppercase tracking-wide text-gray-500">{area}</h2>
                <div className="space-y-2">
                  {clinicalScores.filter((score) => score.area === area).map((score) => (
                    <button
                      key={score.id}
                      onClick={() => switchScore(score)}
                    ...
                    >
                      <span className="block font-semibold">{score.name}</span>
                      <span className={`block text-xs ${activeScore.id === score.id ? 'text-teal-50' : 'text-gray-500'}`}>
                        {score.description}
                      </span>
                    </button>
                  ))}
          ...
          <main className="space-y-4">
            <section className="rounded-3xl border border-gray-200 bg-white p-5 shadow-sm">
              <div className="mb-5 flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                <div>
                  <h2 className="text-xl font-bold text-gray-900">{activeScore.name}</h2>
                  <p className="text-sm text-gray-500">{activeScore.description}</p>
                </div>
              ...
              <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_360px]">
                <div className="space-y-4">
                  {activeScore.fields.map((field) => {
                    const value = fieldValue(field, values);
                    return (
                      <div key={field.id} className="rounded-2xl border border-gray-100 bg-gray-50 p-4">
                        <div className="mb-2 flex items-start justify-between gap-3">
                          <div>
                            <label className="text-sm font-semibold text-gray-800">{field.label}</label>
                            {field.help && <p className="mt-0.5 text-xs text-gray-500">{field.help}</p>}
                          </div>
                          <span className="rounded-lg bg-white px-2 py-1 text-xs font-semibold text-gray-700 shadow-sm">{value}</span>
                        </div>
                        <ScoreInput field={field} value={value} onChange={(next) => updateField(field, next)} />
                      </div>
                    );
                  })}
                </div>
                ...
                <div className="space-y-4">
                  <div className="rounded-2xl border border-gray-100 bg-gray-50 p-4">
                    <p className="mb-3 text-xs font-semibold uppercase tracking-wide text-gray-500">Distribuição dos componentes</p>
                    <div className="h-56">
                      <ResponsiveContainer width="100%" height="100%">
                        <BarChart data={chartData} layout="vertical" margin={{ left: 8, right: 12, top: 4, bottom: 4 }}>
                          ...
                          <Tooltip formatter={(value, _name, item) => [`${item.payload.value}`, 'Valor']} />
                          <Bar dataKey="normalized" radius={[0, 8, 8, 0]} fill="#0d9488" />
                        </BarChart>
                      </ResponsiveContainer>
                    </div>
                  </div>
                  ...
                  {result !== null && interpretation && (
                    <div className={`rounded-2xl border p-4 ${severityClass(interpretation.severity)}`}>
                      <p className="text-xs font-semibold uppercase tracking-wide opacity-80">Resultado</p>
                      <div className="mt-1 flex items-end justify-between gap-3">
                        <span className="text-4xl font-bold tabular-nums">{result}</span>
                        <span className="text-sm font-bold">{interpretation.label}</span>
                      </div>
                      <p className="mt-3 text-sm leading-relaxed">{interpretation.clinicalNote}</p>
                    </div>
                  )}
                  ...
                  <div className="flex gap-3">
                    <button
                      onClick={calculate}
                      className="flex-1 rounded-2xl bg-teal-600 px-4 py-3 text-sm font-semibold text-white shadow-sm transition hover:bg-teal-700"
                    >
                      Calcular
                    </button>
                    {result !== null && patientId && (
                      <button
                        ...
                        className="rounded-2xl border border-teal-300 px-4 py-3 text-sm font-semibold text-teal-700 transition hover:bg-teal-50 disabled:opacity-50"
                      >
            ...
            <section className="rounded-3xl border border-gray-200 bg-white p-5 shadow-sm">
              ...
              <ul className="space-y-2 text-sm text-gray-600">
                {activeScore.references.map((reference) => (
              ...
              <p className="mt-4 rounded-2xl bg-amber-50 p-3 text-xs leading-relaxed text-amber-800">
                Ferramenta de apoio. Critérios classificatórios não equivalem automaticamente a diagnóstico individual; sempre correlacionar com história, exame, exames complementares e diferenciais.
              </p>
            </section>
          </main>
        </div>
